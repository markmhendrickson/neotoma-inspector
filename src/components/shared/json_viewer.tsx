...
function JsonNode({ value, defaultExpanded, depth }: { value: unknown; defaultExpanded: boolean; depth: number }) {
          ...
          <div className="ml-4 border-l pl-2">
            {value.map((item, i) => (
              <div key={i} className="py-0.5">
                ...
                <JsonNode value={item} defaultExpanded={false} depth={depth + 1} />
              </div>
            ))}
          </div>
        )}
          ...
          <div className="ml-4 border-l pl-2">
            {entries.map(([key, val]) => (
              <div key={key} className="py-0.5">
                ...
                <JsonNode value={val} defaultExpanded={false} depth={depth + 1} />
              </div>
            ))}
          </div>
        )}
